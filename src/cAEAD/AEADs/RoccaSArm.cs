...
internal static class RoccaSArm
{
    ...
    private static void Finalization(Span<byte> tag, ulong associatedDataLength, ulong plaintextLength)
    {
        ...
        WriteUInt128LittleEndian(ad, associatedDataLength * 8);
        WriteUInt128LittleEndian(p, plaintextLength * 8);
    ...
    // Taken from https://github.com/dotnet/runtime/blob/main/src/libraries/System.Private.CoreLib/src/System/Buffers/Binary/BinaryPrimitives.WriteLittleEndian.cs
    // since it's only in .NET 8
    private static void WriteUInt128LittleEndian(Span<byte> destination, UInt128 value)
    {
        if (!BitConverter.IsLittleEndian) {
            var tmp = new UInt128(ReverseEndianness((ulong)value), ReverseEndianness((ulong)(value >> 64)));
    ...
    // Taken from https://github.com/dotnet/runtime/blob/main/src/libraries/System.Private.CoreLib/src/System/Buffers/Binary/BinaryPrimitives.ReverseEndianness.cs
    // since it's only in .NET 8
    ...
    {
        return BitOperations.RotateRight(value & 0x00FF00FFu, 8) + BitOperations.RotateLeft(value & 0xFF00FF00u, 8);
    }
}
