...
public static class Ascon80pqc
{
    ...
    public static void Encrypt(Span<byte> ciphertext, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> key, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        x3 ^= k1;
        x4 ^= k2;
        BinaryPrimitives.WriteUInt64BigEndian(ciphertext[^TagSize..^(TagSize - 8)], x2 << 32);
        BinaryPrimitives.WriteUInt64BigEndian(ciphertext[^(TagSize - 4)..^(TagSize - 12)], x3);
        BinaryPrimitives.WriteUInt64BigEndian(ciphertext[^(TagSize - 12)..], x4);
        ZeroState();
    ...
    public static void Decrypt(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> key, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        BinaryPrimitives.WriteUInt64BigEndian(tag[..8], x2 << 32);
        BinaryPrimitives.WriteUInt64BigEndian(tag[4..12], x3);
        BinaryPrimitives.WriteUInt64BigEndian(tag[12..], x4);
