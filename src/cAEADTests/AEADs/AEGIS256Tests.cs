...
public class AEGIS256Tests
{
    ...
    [TestMethod]
    public void ComputeTestVectors()
    {
        Span<byte> c = stackalloc byte[AEGIS256.MaxTagSize];
        Span<byte> m = Convert.FromHexString("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f");
        Span<byte> n = new byte[AEGIS256.NonceSize];
        Span<byte> k = new byte[AEGIS256.KeySize];

        AEGIS256.Encrypt(c, ReadOnlySpan<byte>.Empty, n, k, m[..33], AEGIS256.MaxTagSize);

        string hex1 = Convert.ToHexString(c).ToLower();

        Span<byte> c2 = stackalloc byte[AEGIS256.MinTagSize];
        AEGIS256.Encrypt(c2, ReadOnlySpan<byte>.Empty, n, k, m[..33], AEGIS256.MinTagSize);
        string hex2 = Convert.ToHexString(c2).ToLower();

        string message = Convert.ToHexString(m[..33]).ToLower();
    }
