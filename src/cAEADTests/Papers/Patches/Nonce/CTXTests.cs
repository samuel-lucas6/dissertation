...
{
    // https://github.com/samuel-lucas6/CTX.NET/blob/main/src/CTXDotNetTests/CTXTests.cs
    ...
    {
        Assert.AreEqual(32, CTX.KeySize);
        Assert.AreEqual(12, CTX.NonceSize);
        Assert.AreEqual(20, CTX.TagSize);
    }
    ...
    public void Encrypt_Invalid(int ciphertextSize, int plaintextSize, int nonceSize, int keySize, int associatedDataSize)
    {
        ...
        Assert.ThrowsException<ArgumentOutOfRangeException>(() => CTX.Encrypt(c, p, n, k, ad));
    }
    ...
    public void Decrypt_Invalid(int ciphertextSize, int plaintextSize, int nonceSize, int keySize, int associatedDataSize)
    {
        ...
        Assert.ThrowsException<ArgumentOutOfRangeException>(() => CTX.Decrypt(p, c, n, k, ad));
    }
}
